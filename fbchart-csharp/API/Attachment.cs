...
namespace fbchart_csharp.API
{
    ...
    public class FB_ShareAttachment : FB_Attachment
    {
        ...
        public FB_ShareAttachment(string uid = null, string author = null, string url = null,
            string original_url = null, string title = null, string description = null, string source = null,
            string image_url = null, string original_image_url = null, int image_width = 0, int image_height = 0,
            List<FB_Attachment> attachments = null) : base(uid)
        {
            this.author = author;
            this.url = url;
        ...
        public static FB_ShareAttachment _from_graphql(JToken data)
        {
            ...
            rtn.attachments = data.get("subattachments")?.Select(node => FB_Attachment.graphql_to_subattachment(node))
                ?.ToList();
            ...
            if (media != null && media.get("image") != null)
            {
                ...
                rtn.original_image_url = (rtn.image_url?.Contains("/safe_image.php") ?? false)
                    ? Utils.get_url_parameter(rtn.image_url, "url")
                    : rtn.image_url;
