...
namespace fbchart_csharp.API
{
    ...
    public class FB_Plan
    {
        ...
        public FB_Plan(string uid = null, string time = null, string title = null, string location = null,
            string location_id = null, string author_id = null, Dictionary<string, GuestStatus> guests = null)
        {
            this.uid = uid;
            this.time = time;
            this.title = title;
        ...
        {
            get { return this.guests.Where((g) => g.Value == GuestStatus.GOING).Select((g) => g.Key).ToList(); }
        }

        /// List of the `User` IDs who won't take part in the plan.
        public List<string> declined
        {
            get { return this.guests.Where((g) => g.Value == GuestStatus.DECLINED).Select((g) => g.Key).ToList(); }
        }

        /// List of the `User` IDs who are invited to the plan.
        public List<string> invited
        {
            get { return this.guests.Where((g) => g.Value == GuestStatus.INVITED).Select((g) => g.Key).ToList(); }
        }
        ...
        public static FB_Plan _from_pull(JToken data)
        {
            ...
            rtn.guests = JToken.Parse(data.get("guest_state_list")?.Value<string>()).Select((x) =>
            {
                return new
                {
                    Key = x.get("node")?.get("id")?.Value<string>(),
                    Value = FB_Plan_Constants.GUESTS[x.get("guest_list_state")?.Value<string>()]
                };
